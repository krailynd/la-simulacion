...
export function ProjectileMotionSimulation() {
  ...
  const [selectedHistoryItem, setSelectedHistoryItem] = useState<number | null>(
    null,
  ); // Item seleccionado del historial
  ...
  const range =
    (initialVelocity[0] * initialVelocity[0] * Math.sin(2 * angleRad)) /
    gravity[0]; // Alcance máximo
  ...
  const loadFromHistory = (result: CalculationResult) => {
    ...
    const animateToHistory = () => {
      ...
      if (progress <= 1) {
        ...
        const currentAngle =
          startAngle + (result.angle - startAngle) * progress;
        const currentMass = startMass + (result.mass - startMass) * progress;
        const currentDiameter =
          startDiameter + (result.diameter - startDiameter) * progress;
        const currentDrag =
          startDrag + (result.dragCoeff - startDrag) * progress;
        const currentAltitude =
          startAltitude + (result.altitude - startAltitude) * progress;
    ...
    if (isPlaying && projectile) {
      const animate = () => {
        setProjectile((prev) => {
          ...
          const newY =
            GROUND_Y -
            (vy0 * newTime - 0.5 * gravity[0] * newTime * newTime) * SCALE_Y; // y = y₀ + vy*t - ½gt²
  ...
  useEffect(() => {
    ...
    if (projectile) {
      ...
      if (showVelocityVectors) {
        ...
        ctx.lineTo(
          projectile.x + currentVx * velocityScale,
          projectile.y - currentVy * velocityScale,
        ); // Hasta el extremo del vector
        ctx.stroke();
    ...
    if (projectile) {
      ...
      ctx.fillText(
        `Posición: (${currentRange.toFixed(1)}, ${currentHeight.toFixed(1)}) m`,
        20,
        110,
      );
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card
          className={`lg:col-span-2 ${isDarkMode ? "bg-slate-800 border-slate-700" : "bg-white"}`}
        >
          <CardHeader>
            <CardTitle
          ...
          <CardContent>
            ...
            <div
              className={`mt-4 p-3 rounded-lg ${isDarkMode ? "bg-slate-700" : "bg-gray-100"}`}
            >
              <img
                src="https://www.neurochispas.com/wp-content/uploads/2024/05/Formula-del-rango-en-movimiento-de-proyectiles.png"
                alt="Simulación PhET de movimiento de proyectiles"
                className="w-full h-auto mx-auto rounded"
              />
              <p
                className={`text-xs text-center mt-2 ${isDarkMode ? "text-slate-400" : "text-gray-600"}`}
              >
        ...
        <Card
          className={isDarkMode ? "bg-slate-800 border-slate-700" : "bg-white"}
        >
          <CardHeader>
            <CardTitle className={isDarkMode ? "text-white" : "text-gray-900"}>
              Controles Estilo PhET
            </CardTitle>
          </CardHeader>
        ...
        <CardContent>
          <div className="max-h-64 overflow-y-auto space-y-2">
            {calculationHistory.length === 0 ? (
              ...
              >
                No hay lanzamientos en el historial. Realiza un lanzamiento para
                generar resultados.
              </p>
            ) : (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div
                        className={`text-sm font-medium ${isDarkMode ? "text-white" : "text-gray-900"}`}
                      >
                        v₀ = {result.velocity}m/s, θ = {result.angle.toFixed(1)}
                        °, m = {result.mass}kg
                      </div>
